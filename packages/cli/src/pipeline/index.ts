...
import type { FileInfo, KnowledgeGraph, RepoMetadata, ParsedSymbol } from '../types.js';
...
export async function runPipeline(options: AnalyzeOptions): Promise<{
  graph: KnowledgeGraph;
  repoMetadata: RepoMetadata;
}> {
  const { path: repoPath, verbose } = options;

  if (verbose) console.log(`[pipeline] Starting analysis of: ${repoPath}`);

  // Phase 1: Scan
  if (verbose) console.log('[pipeline:scan] Walking directory tree...');
  const files = scanDirectory(repoPath);
  if (verbose) console.log(`[pipeline:scan] Found ${files.length} files`);
  ...
  // Phase 2: Parse
  if (verbose) console.log('[pipeline:parse] Parsing code with Babel AST...');
  ...
  // Phase 3: Build graph
  if (verbose) console.log('[pipeline:build] Constructing knowledge graph...');
  ...
  // Add symbol nodes
  const allSymbolsByFile = new Map<string, ParsedSymbol[]>();
  ...
  if (verbose) console.log('[pipeline:resolve] Resolving cross-file references...');
  ...
  if (verbose) console.log(`[pipeline:store] Saving to ${dbPath}...`);
  ...
  if (verbose) console.log('[pipeline] Done!');
