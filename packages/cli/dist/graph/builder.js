export class GraphBuilder {
    ...
    symbolMap = new Map();
    filePathMap = new Map(); // normalized path -> file node id
    classSymbolMap = new Map(); // class name -> class symbol
    ...
    addFolder(filePath, name, parentId) {
    ...
    addRelationship(fromId, toId, type, confidence) {
        // Avoid self-loops and duplicate relationships
        if (fromId === toId)
            return '';
        const existing = Array.from(this.relationships.values()).find(r => r.fromId === fromId && r.toId === toId && r.type === type);
        if (existing)
            return existing.id;
        const id = this.makeUniqueId(`rel_${fromId}_${type}_${toId}`);
        const rel = { id, fromId, toId, type, confidence };
        this.relationships.set(id, rel);
        return id;
    }
    resolveImport(importStmt, allSymbols, allImports) {
    ...
    resolveHeritage(allHeritage) {
        for (const h of allHeritage) {
            const classNode = Array.from(this.nodes.values()).find(n => n.label === 'Class' &&
            ...
            if (h.extendsType) {
                const parentNode = Array.from(this.nodes.values()).find(n => n.label === 'Class' && n.properties.name === h.extendsType);
                if (parentNode) {
            ...
            for (const implType of h.implementsTypes) {
                const interfaceNode = Array.from(this.nodes.values()).find(n => n.label === 'Interface' && n.properties.name === implType);
                if (interfaceNode) {
                    this.addRelationship(interfaceNode.id, classNode.id, 'IMPLEMENTS', 1.0);
                }
                else {
                    // Try to find as Type
                    const typeNode = Array.from(this.nodes.values()).find(n => n.label === 'Type' && n.properties.name === implType);
                    if (typeNode) {
    ...
    resolveClassMembers(allSymbols, allProperties) {
        ...
        for (const cls of classes) {
            const classNode = Array.from(this.nodes.values()).find(n => n.label === 'Class' && n.id === cls.id);
            if (!classNode)
                continue;
            ...
            for (const method of methods) {
                const methodNode = Array.from(this.nodes.values()).find(n => n.label === 'Method' && n.id === method.id);
                if (methodNode) {
                    this.addRelationship(classNode.id, methodNode.id, 'HAS_METHOD', 1.0);
                }
            }
            for (const prop of properties) {
                const propNode = Array.from(this.nodes.values()).find(n => n.label === 'Property' && n.id === prop.id);
                if (propNode) {
    ...
    resolveCalls(calls, allSymbols) {
        for (const call of calls) {
            // Find the caller symbol (if any)
            const callerSymbols = allSymbols.filter(s => s.filePath === call.callerFile &&
            ...
            if (callerId) {
                const callerNode = Array.from(this.nodes.values()).find(n => n.properties.filePath === call.callerFile &&
